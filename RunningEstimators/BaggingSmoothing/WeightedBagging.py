...
def outofbag_weighted_bagging_skdim(estimator, Q, X, n_bags=10, k=10, sampling_rate=None, progress_bar=False, estimators=None,
                         estimator_names=None, paralell_estimation=False, weighing_type='0', use_w='n', t=2, pre_smooth=False, geo=None, post_smooth=False, error_type='diff'):
    ...
    def sample_or_split_data_with_indices(X, n_bags=10, sampling_rate=None):
        ...
        if sampling_rate is None:
            ...
            n_samples = int(sampling_rate * len(X))
            bags = []
            split_indices = []
            out_of_bag_indices = []

            for _ in range(n_bags):
                chosen_idx = np.random.choice(indices, size=n_samples, replace=False)
...
def outofbag_weighted_inside_bagging_skdim(estimator, Q, X, n_bags=10, k=10, sampling_rate=None, progress_bar=False, estimators=None,
                         estimator_names=None, paralell_estimation=False, t = 2, pre_smooth=False, geo=None, post_smooth=False, error_type='diff'):
    def compute_distance_matrix(X):
        return squareform(pdist(X))
...
def outofbag_weighted_bagging_LIDkit(estimator, Q, X, n_bags=10, k=10, sampling_rate=None, progress_bar=False, estimators=None,
                         estimator_names=None, paralell_estimation=False, weighing_type='0', use_w='n', t=2, pre_smooth=False, geo=None, post_smooth=False, error_type='diff'):
    def compute_distance_matrix(X):
        return squareform(pdist(X))
    ...
    def results_aggregating(estimate_dictionary, estimator_names, out_of_bag_estimate_dictionary, k_1_dict=None, k_2_dict=None, weighing_type='0', t=2, error_type='diff'):
        if weighing_type == '0':
            if error_type == 'diff':
                test_errors_dictionary = {estimator_names[i]: np.abs(estimate_dictionary[estimator_names[i]] - out_of_bag_estimate_dictionary[estimator_names[i]])**t for i in range(len(estimator_names))}
            elif error_type == 'log_diff':
    ...
    if use_w == 'n':
        if (len(estimator_names) == 1) and (estimator_names[0] == 'mle') and (estimator is None):
            ...
            estimate_dictionary = {estimator_names[i]: np.array([estimates_per_bag_out_of_bag[estimator_names[i]][0][j].lids for j in range(n_bags)]).T for i in range(len(estimator_names))}
            out_of_bag_estimate_dictionary = {estimator_names[i]: np.array([estimates_per_bag_out_of_bag[estimator_names[i]][1][j].lids for j in range(n_bags)]).T for i in range(len(estimator_names))}
        else:
        ...
        bagging_estimators_dictionary, avg_bagging_estimator_dictionary = results_aggregating(estimate_dictionary=estimate_dictionary,
                                                                                              estimator_names=estimator_names,
                                                                                              out_of_bag_estimate_dictionary=out_of_bag_estimate_dictionary,
                                                                                              k_1_dict=None, k_2_dict=None, weighing_type=weighing_type, t=t, error_type=error_type)
    elif use_w != 'y':
        if (len(estimator_names) == 1) and (estimator_names[0] == 'mle') and (estimator is None):
            estimates_per_bag_out_of_bag = {
                estimator_names[i]: LIDkit_estimator.per_bag_and_out_of_bag_estimate(query_points=Q, reference=X,
                                                                                     pooling_method='linear',
                                                                                     loss_function='quadratic', use_w = 'y_approx', pre_smooth= pre_smooth) for i in
                range(len(estimator_names))}
            estimate_dictionary = {estimator_names[i]: np.array(
                [estimates_per_bag_out_of_bag[estimator_names[i]][0][j].lids for j in range(n_bags)]).T for i in
                                   range(len(estimator_names))}
            out_of_bag_estimate_dictionary = {estimator_names[i]: np.array(
                [estimates_per_bag_out_of_bag[estimator_names[i]][1][j].lids for j in range(n_bags)]).T for i in
                                              range(len(estimator_names))}
        else:
            NotImplementedError('Wrong estimator type safeguard. Only mle works for now.')

        bagging_estimators_dictionary, avg_bagging_estimator_dictionary = results_aggregating(
