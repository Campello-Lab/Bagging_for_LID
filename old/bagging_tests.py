...
# Define paths to multiple Git-cloned folders
cloned_folders = [
...
# Now you can import from them normally
#import datasets
...
def plot_knn_graph(x, labels, k=5, title="Geodesic kNN Graph", show = False, save = True, save_name='knn_graph'):
    x = np.asarray(x)
    labels = np.asarray(labels)
    n = len(x)
    # Step 1: Compute k-nearest neighbors
    dists, neighbors = skdim._commonfuncs.get_nn(x, k=k, n_jobs=1)
    # Step 2: Compute pairwise distances and embed in 2D with MDS
    ...
    for i in range(n):
        ...
        for j in neighbors[i]:  # skip self
            color = 'lightgray' if labels[i] == labels[j] else 'dimgray'
            G.add_edge(i, j, color=color)
    # Step 4: Plot the graph
    edge_colors = [G[u][v]['color'] for u, v in G.edges()]
    node_colors = plt.cm.tab10((labels % 10).astype(int))
    plt.figure(figsize=(8, 8))
    nx.draw_networkx_edges(G, pos_dict, edge_color=edge_colors, width=0.5, alpha=0.6)
    nx.draw_networkx_nodes(G, pos_dict, node_color=node_colors, node_size=10, alpha=0.9)
    plt.title(title)
    plt.axis("off")
    plt.tight_layout()
    if save:
...
def generate_touching_halfspaces(
    subspaces=[(1, 1000), (2, 1000), (2, 1000), (3, 1000)],
    ambient_dim=5,
    touch_point=None,
    scale=1.0,
    proximity=0.1,
    shared_groups=None,  # e.g., [[1, 2, 3]]
    seed=0
):
    """
    Generate manifolds with optional shared line groups:
    - Manifolds listed in the same group will share a 1D direction.
    - Others will be independent and only touch at the point.
    """
    np.random.seed(seed)
    X_list = []
    dim_labels = []
    ...
    if touch_point is None:
        touch_point = np.zeros(ambient_dim)

    n_subspaces = len(subspaces)

    # Map from index to group-shared vector if applicable
    index_to_shared_vec = {}

    if shared_groups:
        for group in shared_groups:
            shared_vec = np.random.randn(ambient_dim)
            shared_vec /= np.linalg.norm(shared_vec)
            for idx in group:
                if idx < n_subspaces:
                    index_to_shared_vec[idx] = shared_vec

    # Independent direction for any 1D manifold not in a shared group
    fallback_1d_vec = np.random.randn(ambient_dim)
    fallback_1d_vec /= np.linalg.norm(fallback_1d_vec)

    for i, (d, count) in enumerate(subspaces):
        if i in index_to_shared_vec:
            shared_vec = index_to_shared_vec[i]
            ortho_vectors = []

            while len(ortho_vectors) < d - 1:
                vec = np.random.randn(ambient_dim)
                for prev in [shared_vec] + ortho_vectors:
                    vec -= np.dot(vec, prev) * prev
                norm = np.linalg.norm(vec)
                if norm > 1e-6:
                    ortho_vectors.append(vec / norm)

            basis = np.column_stack([shared_vec] + ortho_vectors)

        elif d == 1:
            # If not in shared group, use separate 1D basis
            basis = fallback_1d_vec[:, None]
        else:
            # Fully independent basis
            basis = np.linalg.qr(np.random.randn(ambient_dim, d))[0]

        # Default scale factors (1 for all directions)
        direction_scales = np.ones(d)

        if i in index_to_shared_vec:
            # Apply smaller scaling to non-shared directions
            direction_scales[1:] = proximity  # Only scale down non-shared directions
        else:
            # Fully independent manifolds → shrink all directions uniformly
            direction_scales[:] = proximity

        # Sample uniformly in unit box
        coords = np.random.rand(count, d) * scale

        # Shrink selected directions
        coords *= direction_scales  # element-wise scaling per coordinate
        points = coords @ basis.T + touch_point

        X_list.append(points)
        dim_labels.append(np.full(count, d))
        subspace_labels.append(np.full(count, i))

    X = np.vstack(X_list)
    intrinsic_dims = np.concatenate(dim_labels)
    labels = np.concatenate(subspace_labels)

    return X, intrinsic_dims

def twod_generate_touching_halfspaces(n, seed=0):
    X, intrinsic_dims = generate_touching_halfspaces(
        subspaces=[(1, int(n/2)), (2, int(n/2))],  # (intrinsic_dim, num_points)
        ambient_dim=4,
        touch_point=None,
        shared_groups=None,  # e.g., [[1, 2, 3]]
        scale=1.0,
        proximity=0.1,  # Smaller = more mass near shared region (thin manifolds)
        seed=0
    )
    return X, intrinsic_dims

def threed_generate_touching_halfspaces(n, seed=0):
    X, intrinsic_dims = generate_touching_halfspaces(
        subspaces=[(1, int(n/3)), (2, int(n/3)), (3, int(n/3))],  # (intrinsic_dim, num_points)
        ambient_dim=4,
        touch_point=None,
        shared_groups=[[1, 2]],
        scale=1.0,
...
def twoplanes_generate_touching_halfspaces(n, seed=0):
    X, intrinsic_dims = generate_touching_halfspaces(
        subspaces=[(1, int(n/3)), (2, int(n/3)), (2, int(n/3))],  # (intrinsic_dim, num_points)
        ambient_dim=4,
        touch_point=None,
        shared_groups=[[1, 2]],
        scale=1.0,
...
def simple_bagging(estimator, Q, X, n_bags=10, k=10, progress_bar=False):
    ...
    def k_smallest_nonzero(distances, k=10):
        distances_nonzero = distances[distances != 0]
        if len(distances_nonzero) >= k:
            result = np.partition(distances_nonzero, k)[:k]
        else:
            ValueError('There are less non-zero distances than the given k')
        return result

    def k_smallest_distance(distances, q, indices, k=10):
        considered_distances = distances[q, indices]
        smallest_distances = k_smallest_nonzero(considered_distances, k)
    ...
    if progress_bar:
        for q in tqdm(range(n)):
            for j in range(n_bags):
                smallest_distances = k_smallest_distance(distances, q, split_indices[j], k=k)
                estimates[q, j] = estimator(smallest_distances=smallest_distances)
    else:
        for q in range(n):
            for j in range(n_bags):
                smallest_distances = k_smallest_distance(distances, q, split_indices[j], k=k)
...
def get_datasets_old(used_params = None, n = 2500):
    ...
    pairs = [(key, [data_gen.dict_gen[key](n = n, d = used_params[key][0], dim = used_params[key][1]), used_params[key][0], used_params[key][1]]) for key in used_params]
    result = dict(pairs)
    return result
...
def plot_test(data_sets, result_dictionaries, dict_names, save=True, save_name='comparrison_table', show=False):
    data = {dict_names[i]: result_dictionaries[i] for i in range(len(dict_names))}
    df_data = {}
    min_mse = {}
    min_bias = {}
    min_var = {}
    for key in next(iter(data.values())).keys():  # Iterate over row labels (same for all dicts)
        values_per_row = [values[key] for values in data.values()]
        mse_values = [v[1] for v in values_per_row]   # MSE
        bias_values = [v[2] for v in values_per_row]  # Bias²
        var_values = [v[3] for v in values_per_row]   # Variance
        min_mse[key] = min(mse_values)
        min_bias[key] = min(bias_values)
        min_var[key] = min(var_values)
    for col, values in data.items():
        ...
        for k, v in values.items():
            ...
            var_value = f"{v[3]:.3f}"
            if v[1] == min_mse[k]:
                mse_value = f'\\textcolor{{red}}{{{mse_value}}}'
            if v[2] == min_bias[k]:
                bias_value = f'\\textcolor{{red}}{{{bias_value}}}'
            if v[3] == min_var[k]:
                var_value = f'\\textcolor{{red}}{{{var_value}}}'
            dicti[k] = (
                f"{mse_value}\n"
                f"{avg_value} {bias_value}\n"
                f"{var_value}"
            )
        df_data[col] = dicti
    df = pd.DataFrame(df_data)
    d_value = [data_sets[key][1] for key in data_sets]
    m_value = [data_sets[key][2] for key in data_sets]
    df.insert(0, 'd', d_value)
    df.insert(1, 'm', m_value)
    fig, ax = plt.subplots(figsize=(2+int(1.5 * len(result_dictionaries)), 6))
    ax.axis('tight')
    ax.axis('off')
    col_labels = [f"{split_long_name(name)}\nMSE\nAVG est. Bias²\nVar"for name in df.columns]
    col_labels[0] = 'd'
    col_labels[1] = 'm'
    # Table data including row names
    table_data = []
    for idx, row in df.iterrows():
        table_data.append([idx] + row.tolist())  # Row label + formatted values
    table = ax.table(
        cellText=table_data,
        colLabels=[""] + col_labels,
        cellLoc='right',  # Default left-align everything
        loc='center'
    )
    for i, label in enumerate(col_labels):
        for key, cell in table.get_celld().items():
            if key[0] == 0:  # Header row
                if i == 0 or i == 1:  # First two columns (d, m) stay centered
                    cell.set_text_props(ha='center')
                else:  # The rest of the columns should be right-aligned
                    cell.set_text_props(ha='right')
    table.auto_set_column_width([0, 1, 2])
    # Adjust font size and spacing
    table.auto_set_font_size(False)
    table.set_fontsize(10)
    table.scale(2, 5)  # Increase row height for readability
    if save:
        directory = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\plots'
        plt.savefig(directory + '\\' + f'{save_name}.pdf', bbox_inches='tight')
    if show:
...
def normalized_scoring(data_sets, dictionaries, names, save=True, save_name='score_table'):
    metrics = ['MSE', 'Bias²', 'Var']
    metric_dict = {}
    set_names = list(data_sets.keys())
    num_set_names = len(set_names)

    # Collecting metric data
    for i in range(len(metrics)):
        metric_array = np.empty((len(dictionaries), len(dictionaries[0])))
        for j in range(len(dictionaries)):
            metric_array[j, :] = np.array([dictionaries[j][set_name][i + 1] for set_name in set_names])
        metric_dict[metrics[i]] = metric_array

    # Normalizing metrics
    normalized_metric_dict = {}
    for metric in metrics:
        normalized_metric_dict[metric] = np.array(
            [normalize(metric_dict[metric][:, i]) for i in range(num_set_names)]).T

    # Calculating average scores
    average_score_dict = {}
    for metric in metrics:
        average_score_dict[metric] = np.mean(normalized_metric_dict[metric], axis=1)
    for metric in metrics:
        won_scores = []
        for i, row in enumerate(normalized_metric_dict[metric]):
            # Count how many times the method's value in the row is the highest across columns (datasets)
            won_count = int(np.sum(row == np.max(normalized_metric_dict[metric], axis=0)))  # max across columns
            won_scores.append(won_count)
        average_score_dict[f'{metric}_won'] = np.array(won_scores, dtype=int)
    names = [split_long_name(name, max_length=16) for name in names]
    df = pd.DataFrame.from_dict(average_score_dict, orient="index", columns=names)
    df = df.T  # Transpose for better table visualization
    ...
    # **Adjust figure size dynamically for large tables**
    row_count = len(df)
    col_count = len(df.columns)
    fig_width = max(9, 2 + col_count * 2)  # Adjust width based on columns
    fig_height = max(9, row_count * 0.6)  # Adjust height based on rows
    ...
    # **Generate table text**
    cell_text = [[f"{v:.2f}" for v in row] for _, row in df.iterrows()]
    ...
    # **Find max in each column and color it red**
    for j, col in enumerate(df.columns):  # Iterate over columns
        max_val = df[col].max()  # Find max in column
        for i, value in enumerate(df[col]):  # Iterate over column values
            if value == max_val:
                table[i + 1, j].get_text().set_color("red")  # Color the max value in red

    # **Adjust row heights to accommodate multiline headers**
    for j, key in enumerate(df.columns):
        table[0, j].set_height(0.2 * len(key.split("\n")))  # Adjust header height

    # **Adjust row heights for data cells**
    for (i, j), cell in table.get_celld().items():
        cell.set_height(0.2)  # Increase if necessary
    ...
    for (i, j), cell in table.get_celld().items():
        if i == 0:  # Column headers
            ...
            cell.set_fontsize(32)  # Increase font size for numerical values
    table.scale(1.0, 1.0)  # Reduce scaling to avoid oversized cells

    # **Save table with bbox_inches="tight" to include all rows**
    directory = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\plots'
    plt.savefig(directory + '\\' + f'{save_name}.pdf', bbox_inches="tight")
...
def skdim_estimators(data_set, method_name, method_type=None, n_bags=10, k=10, sampling_rate = 0.5, progress_bar=False):
    if method_type is None:
        method_type = ''
    if method_name == 'mle':
        if method_type == '':
            estimators, avg_estimator = sk_MLE_full(data_set, k=k)
        elif method_type == 'bag':
            estimators, avg_estimator = simple_bagging_skdim(sk_MLE, data_set, data_set, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=progress_bar)
        elif method_type == 'bag_w':
            estimators, avg_estimator = outofbag_weighted_bagging_skdim(sk_MLE, data_set, data_set, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=progress_bar)
    elif method_name == 'mom':
        if method_type == '':
            estimators, avg_estimator = sk_MOM_full(data_set, k = k)
        elif method_type == 'bag':
            estimators, avg_estimator = simple_bagging_skdim(sk_MOM, data_set, data_set, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=progress_bar)
        elif method_type == 'bag_w':
            estimators, avg_estimator = outofbag_weighted_bagging_skdim(sk_MOM, data_set, data_set, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=progress_bar)
    elif method_name == 'tle':
        if method_type == '':
            estimators, avg_estimator = sk_TLE_full(data_set, k = k)
        elif method_type == 'bag':
            estimators, avg_estimator = simple_bagging_skdim(sk_TLE, data_set, data_set, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=progress_bar)
        elif method_type == 'bag_w':
            estimators, avg_estimator = outofbag_weighted_bagging_skdim(sk_TLE, data_set, data_set, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=progress_bar)
    elif method_name == 'mle_own':
        if method_type == '':
            estimators, avg_estimator = LID_MLE_Q(data_set, data_set, k=k, p=2, progress_bar=progress_bar)
        elif method_type == 'bag':
            estimators, avg_estimator = simple_bagging(MLE, data_set, data_set, n_bags=n_bags, k=k, progress_bar=progress_bar)
    return estimators, avg_estimator
...
def run_method(data_sets, method_name, method_type, k=10, n_bags=10, sampling_rate = 0.5, save=True):
    estimators = dict([(key, skdim_estimators(data_sets[key][0], method_name=method_name, method_type=method_type, n_bags=n_bags, k=k, sampling_rate = sampling_rate, progress_bar=False)) for key in tqdm(data_sets)])
    estimators_results = get_comparrison_measures(data_sets, estimators)
    if save:
        save_to_df(estimators_results, f'{method_name}_{method_type}_results')
    return estimators, estimators_results
...
def run_test(data_sets, ks=[10], n_bagss=[10], sampling_rates = [0.5], save=True, method_names=['mle', 'mom', 'tle'], method_types = ['', 'bag', 'bag_w'], pairs = [(10, 10, 0.5)], use_pairs = True):
    results = {}
    if not use_pairs:
        for k in ks:
            for method_name in method_names:
                for method_type in method_types:
                    if method_type == 'bag' or method_type == 'bag_w':
                        for n_bags in n_bagss:
                            for sampling_rate in sampling_rates:
                                results[f'{method_name}_{method_type}_k_{k}_n_bags_{n_bags}_sampling_rate_{sampling_rate}'] = run_method(data_sets, method_name=method_name, method_type=method_type, k=k, n_bags=n_bags, sampling_rate=sampling_rate, save=False)
                    else:
                        results[f'{method_name}_{method_type}_k_{k}'] = run_method(data_sets, method_name=method_name, method_type=method_type, k=k, save=False)
        savename = f"k_{'_'.join(str(x) for x in ks)}_" \
                   f"n_bags_{'_'.join(str(x) for x in n_bagss)}_" \
                   f"sampling_rates_{'_'.join(str(x) for x in sampling_rates)}_" \
                   f"method_names_{'_'.join(str(x) for x in method_names)}_" \
                   f"method_types_{'_'.join(str(x) for x in method_types)}"
    else:
        for method_name in method_names:
            for method_type in method_types:
                if method_type == 'bag' or method_type == 'bag_w':
                    for pair in pairs:
                        k = pair[0]
                        n_bags = pair[1]
                        sampling_rate = pair[2]
                        results[f'{method_name}_{method_type}_k_{k}_n_bags_{n_bags}_sampling_rate_{sampling_rate}'] = run_method(data_sets, method_name=method_name, method_type=method_type, k=k, n_bags=n_bags, sampling_rate=sampling_rate, save=False)
                else:
                    uniques_ks = list(set([pair[0] for pair in pairs]))
                    for k in uniques_ks:
                        results[f'{method_name}_{method_type}_k_{k}'] = run_method(data_sets, method_name=method_name, method_type=method_type, k=k, save=False)
        savename = f"k_n_bags_sampling_rate_{'_'.join(str(x[0]) + '_' + str(x[1])+ '_' + str(x[2]) for x in pairs)}_" \
                   f"method_names_{'_'.join(str(x) for x in method_names)}_" \
                   f"method_types_{'_'.join(str(x) for x in method_types)}"
    if save:
        save_to_df(results, f'{savename}')
    return results

def run_test_fast(data_sets, ks=[10], n_bagss=[10], sampling_rates = [0.5], save=True, method_names=['mle', 'mom', 'tle'], method_types = ['', 'bag', 'bag_w'], pairs = [(10, 10, 0.5)], use_pairs = True):
    results = {}
    if not use_pairs:
        for k in ks:
            for method_type in method_types:
                if method_type == 'bag' or method_type == 'bag_w':
                    for n_bags in n_bagss:
                        for sampling_rate in sampling_rates:
                            use_method_names = [item for item in method_names if item != 'lidl']
                            result_dictionary, estimators_results = run_method_fast(data_sets, use_method_names, method_type, k=k, n_bags=n_bags, sampling_rate=sampling_rate, save=False)
                            for method_name in use_method_names:
                                results[f'{method_name}_{method_type}_k_{k}_n_bags_{n_bags}_sampling_rate_{sampling_rate}'] = (result_dictionary[method_name], estimators_results[method_name])
                else:
                    result_dictionary, estimators_results = run_method_fast(data_sets, method_names, method_type, k=k, save=False)
                    for method_name in method_names:
                        results[f'{method_name}_{method_type}_k_{k}'] = (result_dictionary[method_name], estimators_results[method_name])
        savename = f"k_{'_'.join(str(x) for x in ks)}_" \
                   f"n_bags_{'_'.join(str(x) for x in n_bagss)}_" \
                   f"sampling_rates_{'_'.join(str(x) for x in sampling_rates)}_" \
                   f"method_names_{'_'.join(str(x) for x in method_names)}_" \
        ...
        for method_type in method_types:
            if method_type == 'bag' or method_type == 'bag_w':
                for pair in pairs:
                    ...
                    n_bags = pair[1]
                    sampling_rate = pair[2]
                    result_dictionary, estimators_results = run_method_fast(data_sets, method_names, method_type, k=k,
                                                                            n_bags=n_bags, sampling_rate=sampling_rate,
                                                                            save=False)
                    for method_name in method_names:
                ...
                for k in uniques_ks:
                    ...
                    for method_name in method_names:
                        ...
                        result_dictionary[method_name], estimators_results[method_name])
        savename = f"k_n_bags_sampling_rates_{'_'.join(str(x[0]) + '_' + str(x[1]) + '_' + str(x[2]) for x in pairs)}_" \
                   f"method_names_{'_'.join(str(x) for x in method_names)}_" \
                   f"method_types_{'_'.join(str(x) for x in method_types)}"
    if save:
        save_to_df(results, f'{savename}')
...
# Move run_task function to the top level
def run_task_old(args, data_sets, method_names):
    """Helper function to run `run_method_fast` with the given arguments."""
    k, method_type, n_bags, sampling_rate = args
    results = {}
    if method_type == 'bag' or method_type == 'bag_w':
        result_dictionary, estimators_results = run_method_fast(
            data_sets, method_names, method_type, k=k,
            n_bags=n_bags, sampling_rate=sampling_rate, save=False
        )
        for method_name in method_names:
            results[f'{method_name}_{method_type}_k_{k}_n_bags_{n_bags}_sampling_rate_{sampling_rate}'] = (
                result_dictionary[method_name], estimators_results[method_name]
        ...
        result_dictionary, estimators_results = run_method_fast(
            data_sets, method_names, method_type, k=k, save=False
        )
        for method_name in method_names:
            results[f'{method_name}_{method_type}_k_{k}'] = (
                result_dictionary[method_name], estimators_results[method_name]
...
def run_test_fast_multiprocess_old(data_sets, ks=[10], n_bagss=[10], sampling_rates=[0.5], save=True,
                               method_names=['mle', 'mom', 'tle'], method_types=['', 'bag', 'bag_w'],
                               pairs=[(10, 10, 0.5)], use_pairs=True):
    results = {}
    # Generate all parameter combinations
    param_combinations = []
    for k in ks:
        for method_type in method_types:
            if method_type in ['bag', 'bag_w']:
                for n_bags in n_bagss:
                    for sampling_rate in sampling_rates:
                        param_combinations.append((k, method_type, n_bags, sampling_rate))
            else:
                param_combinations.append((k, method_type, None, None))
    # Run in parallel using multiprocessing
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        # Pass additional arguments to the worker function
        results_list = pool.starmap(run_task_old, [(args, data_sets, method_names) for args in param_combinations])
...
def run_all(n=2500, ks=[10], n_bagss=[10], sampling_rates = [0.5], save=True, method_names=['mle', 'mom', 'tle'], method_types = ['', 'bag', 'bag_w'], save_name='comparrison', load = False, load_path = None, pairs = [(10, 10, 0.5)], use_pairs = True):
    if load:
        results = load_results(load_path, n=n, save=save, save_name=save_name)
    else:
        data_sets = get_datasets(n=n)
        results = run_test(data_sets, ks=ks, n_bagss=n_bagss, sampling_rates = sampling_rates, save=save, method_names=method_names, method_types=method_types, pairs=pairs, use_pairs=use_pairs)
        result_dictionaries, dict_names = convert_results_for_plot(results)
        if use_pairs:
            save_name = f"n_{n}_k_n_bags_sampling_rates_{'_'.join(str(x[0]) + '_' + str(x[1]) + '_' + str(x[2]) for x in pairs)}_" \
                       f"method_names_{'_'.join(str(x) for x in method_names)}_" \
                       f"method_types_{'_'.join(str(x) for x in method_types)}"
        else:
            save_name = f"n_{n}_k_{'_'.join(str(x) for x in ks)}_" \
                       f"n_bags_{'_'.join(str(x) for x in n_bagss)}_" \
                       f"sampling_rates_{'_'.join(str(x) for x in sampling_rates)}_" \
                       f"method_names_{'_'.join(str(x) for x in method_names)}_" \
                       f"method_types_{'_'.join(str(x) for x in method_types)}"
        plot_test(data_sets, result_dictionaries, dict_names, save=save, save_name=f'{save_name}_table')
        normalized_scoring(data_sets, result_dictionaries, dict_names, save=save, save_name=f'{save_name}_score_table')
        create_spider_chart(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, metric='MSE', save=save, save_name=f'{save_name}_spider_chart_MSE_normalized')
        create_spider_chart(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, metric='Bias2', save=save, save_name=f'{save_name}_spider_chart_Bias2_normalized')
        create_spider_chart(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, metric='Var', save=save, save_name=f'{save_name}_spider_chart_Var_normalized')
    return results

def run_all_fast_old(n=2500, ks=[10], n_bagss=[10], sampling_rates = [0.5], save=True, method_names=['mle', 'mom', 'tle'], method_types = ['', 'bag', 'bag_w'], save_name='comparrison', load = False, load_path = None, pairs = [(10, 10, 0.5)], use_pairs = True):
    if load:
        results = load_results(load_path, n=n, save=save, save_name=save_name)
    else:
        data_sets = get_datasets(n=n)
        results = run_test_fast_multiprocess(data_sets, ks=ks, n_bagss=n_bagss, sampling_rates=sampling_rates, save=save, method_names=method_names, method_types=method_types, pairs=pairs, use_pairs=use_pairs)
        result_dictionaries, dict_names = convert_results_for_plot(results)
        if use_pairs:
            save_name = f"n_{n}_k_n_bags_sampling_rates_{'_'.join(str(x[0]) + '_' + str(x[1]) + '_' + str(x[2]) for x in pairs)}_" \
                       f"method_names_{'_'.join(str(x) for x in method_names)}_" \
...
def run_all_fast(param_list, n=2500, save=True, load = False, load_path=None, short_name=''):
    data_sets = get_datasets(n=n)
    if load:
        results = load_dict(load_path)
    else:
        results = run_test_fast_multiprocess(data_sets, param_list=param_list)
    result_dictionaries, dict_names = convert_results_for_plot(results)
    save_name = f"n_{n}_{'_'.join(str(x[0]) + '_' + str(x[1]) + '_' + str(x[2]) + '_' + '_'.join(y for y in x[3]) + '_' + str(x[4]) for x in param_list)}"
    save_name_short = f"n_{n}_{short_name}"
    if save and not load:
        directory = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\pkls'
        save_dict(data=results, directory=directory, filename=save_name)

    #plot_test(data_sets, result_dictionaries, dict_names, save=save, save_name=f'{save_name_short}_table')
    #normalized_scoring(data_sets, result_dictionaries, dict_names, save=save, save_name=f'{save_name_short}_score_table')
    #create_spider_chart(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, metric='MSE', save=save, save_name=f'{save_name_short}_spider_chart_MSE_normalized')
    #create_spider_chart(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, metric='Bias2', save=save, save_name=f'{save_name_short}_spider_chart_Bias2_normalized')
    #create_spider_chart(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, metric='Var', save=save, save_name=f'{save_name_short}_spider_chart_Var_normalized')
    return results

def big_run():
    param_list = [([10, 20], [10, 50], [0.2, 0.4, 0.6], ['mle', 'mom', 'tle', 'mada', 'ess'], ['']),
                  ([10, 20], [10, 50], [0.2, 0.4, 0.6], ['mle', 'mom', 'tle', 'mada', 'ess'], ['bag']),
                  (10, None, None, ['lidl'], [''])]
    ks = [10, 20, 30]
    n_bagss = [10, 20, 30]
    results1 = run_all(n=10000, ks=ks, n_bagss=n_bagss, save=True, method_names=['mle'], method_types=['', 'bag'], save_name='comparrison_1')
    results2 = run_all(n=10000, ks=ks, n_bagss=n_bagss, save=True, method_names=['mom'], method_types=['', 'bag'], save_name='comparrison_2')
    results3 = run_all(n=10000, ks=ks, n_bagss=n_bagss, save=True, method_names=['tle'], method_types=['', 'bag'],save_name='comparrison_3')
    ks = [10]
    n_bagss = [10]
    resultsfull = run_all(n=10000, ks=ks, n_bagss=n_bagss, save=True, method_names=['mle', 'mom', 'tle'], method_types=['', 'bag', 'bag_w'], save_name='comparrison_full')

####################################new#############################################

def plot_k_old(results_list, name_list, save_name='kplot', show = False, log = False):
    ...
    for name, result in zip(name_list, results_list):
        ...
        if not k_match:
            print(f"[WARN] No k found in name: {name}")
            continue
        ...
        if 'mle_bag' in name:
            ...
            if sr_match:
                sampling_rate = sr_match.group(1)
                method_label = f'Bag (rate={sampling_rate})'
            else:
                print(f"[WARN] No sampling rate found in: {name}")
                method_label = 'Bag (rate=unknown)'
        else:
            method_label = 'MLE'

        # Add to dataset-wise structure
    ...
    # Sort all results by k
    for dataset in data_by_dataset:
        for method_label in data_by_dataset[dataset]:
            data_by_dataset[dataset][method_label].sort(key=lambda x: x['k'])
            ks = [entry['k'] for entry in data_by_dataset[dataset][method_label]]

    # Plot function
    def plot_metric(metric_name, ylabel):
        dataset_names = sorted(data_by_dataset.keys())
        fig, axes = plt.subplots(len(dataset_names), 1, figsize=(8, 4 * len(dataset_names)), sharex=True)
        if len(dataset_names) == 1:
            axes = [axes]

        for ax, dataset in zip(axes, dataset_names):
            for method_label, entries in sorted(data_by_dataset[dataset].items()):
                ks = [entry['k'] for entry in entries]
                ys = [entry[metric_name] for entry in entries]
                if not log:
                    ax.plot(ks, ys, label=method_label, marker='o', markersize=3)
                else:
                    ax.plot(ks, np.log(ys), label=method_label, marker='o', markersize=3)
...
def plot_lollipop_error(x, dims, estimates, show = False, save_name='spatial_plot'):
    x_2d = x[:, :2]
    abs_error = np.abs(estimates - dims)
    normalized_error = np.where(dims > 0, abs_error / dims, abs_error)
    plt.figure(figsize=(8, 8))
    ...
    cbar.set_label("Normalized Error (|estimate - true| / true)")
    plt.title("Lollipop Dataset with Normalized Estimation Error")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.grid(True)
    plt.axis("equal")
...
def plot_three_lollipop_datasets(datasets, show = False, save_name='spatial_plot'):
    green_red_cmap = mcolors.LinearSegmentedColormap.from_list("GreenRed", ["green", "red"])
    num_datasets = len(datasets)
    fig, axs = plt.subplots(num_datasets, 1, figsize=(8, 8 * num_datasets), dpi=400)
    # Ensure axs is iterable in case of a single dataset.
    if num_datasets == 1:
        axs = [axs]
    for ax, (key, data) in zip(axs, datasets.items()):
        x, dims, estimates = data
        # Use only the first two dimensions for 2D plotting.
        x_2d = x[:, :2]
        # Compute the absolute error.
        abs_error = np.abs(estimates - dims)
        # Compute normalized error to account for proportional differences:
        # If dims > 0, normalized error = abs_error / dims, else use abs_error directly.
        normalized_error = np.where(dims > 0, abs_error / dims, abs_error)
        # Clip normalized error between 0 and 1. This means:
        # 0 corresponds to 0% error (green) and 1 corresponds to 100% error or more (red).
        normalized_error_clipped = np.clip(normalized_error, 0, 1)
        # Create the scatter plot, using the custom colormap and rasterizing the points to reduce PDF size.
    ...
    plt.tight_layout()
    directory = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\plots'
    plt.savefig(directory + '\\' + f'{save_name}.pdf', dpi=400, bbox_inches="tight")
    if show:
        plt.show()

def plot_dimension_estimation_errors(datasets, show=False, save_name='dimension_error_plot'):
    green_red_cmap = mcolors.LinearSegmentedColormap.from_list("GreenRed", ["green", "red"])
    num_datasets = len(datasets)
    fig, axs = plt.subplots(num_datasets, 1, figsize=(8, 6 * num_datasets), dpi=400)
    if num_datasets == 1:
        axs = [axs]
    for ax, (key, data) in zip(axs, datasets.items()):
        x, true_dims, estimates = data
        # Use first two dimensions for 2D plotting
        x_2d = x[:, :2]
        # Compute absolute and normalized error
        abs_error = np.abs(estimates - true_dims)
        normalized_error = np.where(true_dims > 0, abs_error / true_dims, abs_error)
        normalized_error_clipped = np.clip(normalized_error, 0, 1)
        # Scatter plot
        sc = ax.scatter(x_2d[:, 0], x_2d[:, 1],
                        s=1,
...
def plot_dimension_comparison_grid(results, data_sets, save_name='grid_plot', save_dir=r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\plots', show=False):
    # --- Setup ---
    ...
    fig, axs = plt.subplots(num_methods, num_datasets,
                            figsize=(8 * num_datasets, 7 * num_methods),
                            dpi=400)
    # Make axs always 2D for consistency
    if num_methods == 1:
    ...
    for row_idx, method_key in enumerate(method_keys):
        ...
        for col_idx, dataset_name in enumerate(dataset_names):
            ...
            x_2d = X[:, :2]
            abs_error = np.abs(estimated_dims - true_dims)
            normalized_error = np.where(true_dims > 0, abs_error / true_dims, abs_error)
            normalized_error_clipped = np.clip(normalized_error, 0, 1)
            sc = ax.scatter(x_2d[:, 0], x_2d[:, 1],
                            s=1,
            ...
            if col_idx == 0:
                ax.set_ylabel(method_key, fontsize=9)
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_aspect('equal')
    # --- Colorbar (Shared) ---
...
def plot_dimension_comparison_grid_local(results, data_sets, save_name='grid_plot', save_dir=r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\plots', show=False):
    dataset_names = list(data_sets.keys())
    method_keys = list(results.keys())
    green_red_cmap = mcolors.LinearSegmentedColormap.from_list("GreenRed", ["green", "red"])
    num_methods = len(method_keys)
    ...
    for row_idx, method_key in enumerate(method_keys):
        ...
        for col_idx, dataset_name in enumerate(dataset_names):
            ...
            x_2d = x_2d[mask]
            true_dims = true_dims[mask]
            estimated_dims = estimated_dims[mask]
...
def data_plots(dataset, k = 10, sampling_rate = 0.1, n_bags=10, save_name ='spatial_plot_data', save = True, load=False, load_path = None):
    X = dataset[0]
    dims = dataset[1]
    if load:
        estimdict = load_dict(load_path)
    else:
        lid1, _ = sk_MLE_full(X, k=10, correct=True, dists=None, knnidx=None)
        lid2, _ = simple_bagging_skdim(sk_MLE, X, X, n_bags=n_bags, k=k, sampling_rate=sampling_rate, progress_bar=False, estimators=[sk_MLE], estimator_names=['mle'], paralell_estimation=True)
        lid2 = lid2['mle']
        lid3, _ = outofbag_weighted_bagging_skdim(sk_MLE, X, X, n_bags=n_bags, k=k, sampling_rate=sampling_rate, progress_bar=True,
                                        estimators=[sk_MLE],
                                        estimator_names=['mle'], paralell_estimation=True)
        lid3 = lid3['mle']
        estimdict = {'MLE': (X, dims, lid1), 'MLE-Bag': (X, dims, lid2), 'MLE-Bag-W': (X, dims, lid3)}
    if save and not load:
        directory = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\pkls2'
        save_dict(data=estimdict, directory=directory, filename=f'{save_name}_data')
    plot_three_lollipop_datasets(estimdict, save_name=save_name)
...
def split_dict_in_half(d):
    items = list(d.items())
    half = len(items) // 2
    first_half = dict(items[:half])
    second_half = dict(items[half:])
    return first_half, second_half

if __name__ == "__main__":
    #([50], None, None, ['ess'], [''])
    #param_combs = generate_param_combinations(param_list)
    #param_combs = [([10, 20, 30, 40, 50], None, None, ['ess'], ['']), ([10, 20, 30, 40, 50], [30], [0.1, 0.5, 0.8], ['mle'], ['bag'])]
    #load_path = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\pkls\n_500_20_None_None_ess__5_30_0.05_mle_bag_5_30_0.1_mle_bag_5_30_0.25_mle_bag_5_30_0.5_mle_bag_10_30_0.05_mle_bag_10_30_0.1_mle_bag_10_30_0.25_mle_bag_10_30_0.5_mle_bag_20_30_0.05_mle_bag_20_30_0.1_mle_bag_20_30_0.25_mle_bag_20_30_0.5_mle_bag'
    #resultsfull = run_all_fast(param_list=param_combs, n=10000, save=True, load=False, load_path=load_path, short_name='ess_vs_ess_bagging')
    #results = load_plot(r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\csvs\results_k_10_n_bags_10.csv', n = 2500, save=True, save_name='comparrison_table_LOAD2')
    #['mle', 'mom', 'tle', 'mada', 'ess', 'lidl']
    #load_path = r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\pkls2\grid_plot_with_bar'
    #param_list = [([10], [10], [0.1], ['mle'], ['bag_w_0', 'bag_w_equalizing_k_w', 'bag_w_penalizing_k_w'])] #'', 'bag', 'bag_w_0', '', 'bag', 'bag_w_bag',
    #param_list = [([2 + i for i in range(20)], [10], [0.1], ['mle'], ['', 'bag', 'bag_w_0'])] #'', 'smooth', 'bag', 'bag_w_0'
    #param_list = [([10], [10], [0.1], ['mle'], ['', 'smooth', 'bag', 'bag_w_0'])]
    #param_list = [([50], [10], [0.04 + i * 0.02 for i in range(40)], ['mle'], ['', 'bag'])]
    param_combs = generate_param_combinations(param_list)
    #usedata = [-1-i for i in range(39)]
    # 'bag_w_0', 'bag_w_inf', 'bag_w_equalizing', 'bag_w_penalizing', 'bag_w_p_val_mean', 'bag_w_p_val_raw', 'bag_w_p_val_mean2', 'bag_w_p_val_symmetric'
    #test_types = ['dim', 'region']
    #bounddict = [{0: (1, 1.5), 1: (1, 1.5)}, {0: (1.2, 1.4), 1: (1.2, 1.4)}]
    result_dictionaries, dict_names = new_plots(n=2000, param_list=param_combs, save=True, load=False, load_path=load_path, save_name='mle_bag_type_test_n2000', usedata=usedata, test_types=None, bounddict=None)
    #print(dict_names)
    #allowed_methods = {
    #    'mle': True,
    #    'bag': ['0.5'],  # allow only sampling_rate=0.5 for bag
    #    'bag_w': ['0.1']  # allow only sampling_rate=0.1 for bag_w
    #}
    plot_k(result_dictionaries, dict_names, save_name='kplot_full_new_log10_n3000', log=True, allowed_methods=None)
    plot_bias_variance_bars_varying_n_bags(result_dictionaries, dict_names, 15, 0.025, show=False, save_name='n_bags_bar_plot_sr0.025')
    plot_bias_variance_bars_sr(result_dictionaries, dict_names, fixed_k=50, show=False, save_name='sr_bar_plot_lollipop_k50')
    #load_path =  r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\pkls2\spatial_plot_n10000_k5_data'
    #dataset = lollipop_dataset_0_dense_head(10000)
    #data_plots(dataset, k=10, sampling_rate=0.1, n_bags=10, save_name ='spatial_plot_n10000_k10_Sr0.1', save=True, load=False, load_path=load_path)

    results, data_sets = result_generator(n=2500, param_list=param_combs, save=True, load=False, load_path=load_path, save_name='new_datasets_kplot', usedata=usedata, test_types=None, bounddict=None)
    #result_dictionaries, dict_names = convert_results_for_plot(results)
    plot_k(result_dictionaries, dict_names, save_name='new_datasets_kplot', log=True, allowed_methods=None)
    #create_stacked_spider_charts(data_sets, dictionaries=result_dictionaries, names=dict_names, normalize_data=True, save=True, save_name='stacked_spider_charts5_k_10', fill=True)
    #plot_dimension_comparison_grid_local(results, data_sets, save_name='grid_local_plot_mle_n10000', save_dir=r'C:\Users\User\PycharmProjects\pythonProjec # t3\LIDstuff\saved_results\plots', show=False)
    #plot_dimension_comparison_grid_local_with_mse_bars(results, data_sets, save_name='grid_plot_with_bar',
    #                                                   save_dir=r'C:\Users\User\PycharmProjects\pythonProject3\LIDstuff\saved_results\plots',
    #                                                   show=False)
    dataset_dict = get_datasets(n=500)
    keylist = ['M1_Sphere', 'M2_Affine_3to5', 'M3_Nonlinear_4to6', 'M4_Nonlinear', 'M5b_Helix2d', 'M6_Nonlinear', 'M7_Roll', 'M8_Nonlinear', 'M9_Affine', 'M10a_Cubic', 'M10b_Cubic', 'M10c_Cubic', 'M11_Moebius', 'M12_Norm', 'M13a_Scurve',
     'Mn1_Nonlinear', 'Mn2_Nonlinear', 'lollipop_', 'lollipop_0', 'lollipop_0_dense_head', 'lollipop', 'swiss_roll', 'affine_1000D_100d_uniform',
     'affine_1000D_900d_gaussian', 'affine_1000D_900d_uniform', 'affine_2000D_100d_200d_uniform', 'affine_800D_10d_80d_200d_uniform', 'affine_800D_200d_uniform', 'squiggly_05_freq_2000D_200d_uniform', 'squiggly_10_freq_2000D_200d_uniform', 'squiggly_1_freq_2000D_200d_uniform',
      'squiggly_5_freq_2000D_200d_uniform', 'affine_100D_10d_25d_50d_gaussian', 'affine_100D_10d_25d_50d_uniform', 'affine_100D_10d_30d_90d_uniform',
     'affine_100D_10d_uniform', 'affine_100D_30d_gaussian', 'affine_100D_30d_uniform', 'affine_100D_90d_gaussian', 'affine_100D_90d_uniform', 'squiggly_05_freq_100D_10d_25d_50d_uniform', 'squiggly_05_freq_100D_50d_uniform', 'squiggly_10_freq_100D_10d_25d_50d_uniform',
     'squiggly_10_freq_100D_50d_uniform', 'squiggly_1_freq_100D_10d_25d_50d_uniform', 'squiggly_1_freq_100D_50d_uniform', 'squiggly_5_freq_100D_10d_25d_50d_uniform', 'squiggly_5_freq_100D_50d_uniform', 'affine_10D_2d_4d_8d_gaussian',
     'affine_10D_2d_4d_8d_laplace', 'affine_10D_2d_4d_8d_uniform', 'affine_10D_5d_gaussian', 'affine_10D_5d_laplace', 'affine_10D_5d_uniform',
     'squiggly_05_freq_10D_2d_4d_8d_uniform', 'squiggly_10_freq_10D_2d_4d_8d_uniform', 'squiggly_1_freq_10D_2d_4d_8d_uniform', 'squiggly_5_freq_10D_2d_4d_8d_uniform', 'torus_circle']
    keylist1 = keylist[0:10]
    keylist2 = keylist[10:20]
    keylist3 = keylist[20:30]
    keylist4 = keylist[30:40]
    keylist5 = keylist[40:50]
    keylist6 = keylist[50:]
    dict1 = {key: dataset_dict[key] for key in keylist1}
    dict2 = {key: dataset_dict[key] for key in keylist2}
    dict3 = {key: dataset_dict[key] for key in keylist3}
    dict4 = {key: dataset_dict[key] for key in keylist4}
    dict5 = {key: dataset_dict[key] for key in keylist5}
    dict6 = {key: dataset_dict[key] for key in keylist6}
    plot_all_knn_graphs(dict5, k=3, output_path='all_knn_graphs_5.pdf')
    #plot_all_knn_graphs(second_half1, k=3, output_path='all_knn_graphs_2.pdf')
    #plot_all_knn_graphs(first_half2, k=3, output_path='all_knn_graphs_3.pdf')
    #plot_all_knn_graphs(second_half2, k=3, output_path='all_knn_graphs_4.pdf')
    '''
    ['M1_Sphere', 'M2_Affine_3to5', 'M3_Nonlinear_4to6', 'M4_Nonlinear', 'M5b_Helix2d', 'M6_Nonlinear', 'M7_Roll', 'M8_Nonlinear', 'M9_Affine', 'M10a_Cubic', 'M10b_Cubic', 'M10c_Cubic', 'M11_Moebius', 'M12_Norm', 'M13a_Scurve',
     'Mn1_Nonlinear', 'Mn2_Nonlinear', 'lollipop_', 'lollipop_0', 'lollipop_0_dense_head', 'lollipop', 'swiss_roll', 'affine_1000D_100d_uniform',
     'affine_1000D_900d_gaussian', 'affine_1000D_900d_uniform', 'affine_2000D_100d_200d_uniform', 'affine_800D_10d_80d_200d_uniform', 'affine_800D_200d_uniform', 'squiggly_05_freq_2000D_200d_uniform', 'squiggly_10_freq_2000D_200d_uniform', 'squiggly_1_freq_2000D_200d_uniform',
      'squiggly_5_freq_2000D_200d_uniform', 'affine_100D_10d_25d_50d_gaussian', 'affine_100D_10d_25d_50d_uniform', 'affine_100D_10d_30d_90d_uniform',
     'affine_100D_10d_uniform', 'affine_100D_30d_gaussian', 'affine_100D_30d_uniform', 'affine_100D_90d_gaussian', 'affine_100D_90d_uniform', 'squiggly_05_freq_100D_10d_25d_50d_uniform', 'squiggly_05_freq_100D_50d_uniform', 'squiggly_10_freq_100D_10d_25d_50d_uniform',
